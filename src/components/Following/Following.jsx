...
const Following = () => {
  ...
  if (errorEncounter) {
    return <div className="followers-section"> <h2>Something went wrong! We are unable to get the following request. Please try again later</h2></div>
  }
  ...
  return (
    <section className="following-section">
      ...
      {following.map((currentFollowing, i) => {
        return (<Follow avatar={currentFollowing.avatar_url} key={i} name={currentFollowing.login}/>)
      })}
    </section>
  );
};
